@GW_PC_TC015
Feature: GW_PC_TC015_New Business - Retrieve MVR when new one more driver is added during policy change
    Background:
        Given User is on login page

    @GW_PC_TC015 @Regression @Smoke
    Scenario Outline: To verify that user is able to Retrieve MVR when new one more driver is added during policy change
        ...
        # And User click on Actions Tab and "Navigate to the Policy Change"
        # And User Navigate to the drivers tab and add one more drivers and click on Retrieve MVR
        #     | Firstname | Lastname  | Date of Birth | Address 1 | ZIP Code | Address Type | License No. | License State |
        #     | Test      | NewDriver | 02/07/1996    | 123 ABC   | 85001    | Billing      | A98765433   | Arizona       |
        # Then User should Verify if the user is able to Quote and issue the policy

        Examples:
            | firstname | lastname | Year First Licensed | Offering Selection | Date of Birth | License No. | License State | Year First Licensed | VIN               | VLicense State | CostNew | Year |
            | Test      | Policy   | 2015                | Basic Program      | 02/07/1996    | A98765432   | Arizona       | 2015                | 2FMDK51C99BA83243 | Arizona        | 2000    | 2015 |
